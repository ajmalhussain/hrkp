...
class user extends Base_model {
    ...
    private $conn;
    //db fileds
    protected static $db_fields = array('username', 'designation', 'login_id', 'password', 'email', 'phone','district_of_domicile', 'role_id', 'is_active');
...
//        if ($_SESSION['role'] == 1) {
//            return $this->query("SELECT * FROM " . static::$table_name." INNER JOIN roles ON users.role_id = roles.pk_id");
//        } else {
//            return $this->query("SELECT * FROM " . static::$table_name ." INNER JOIN roles ON users.role_id = roles.pk_id WHERE status = 1");
//        }
       if ($_SESSION['role'] == 1) {
	...
	users.email,
	users.phone,
	users.district_of_domicile,
	locations.LocName location_name,
	users.role_id,
        users.is_active,
	roles.role_name
        FROM
	users
       LEFT JOIN tbl_locations locations ON users.district_of_domicile = locations.PkLocID
       LEFT JOIN roles ON users.role_id = roles.pk_id
	...
	roles.role_name
        FROM
	users
        LEFT JOIN locations ON users.district_of_domicile = locations.pk_id
        LEFT JOIN roles ON users.role_id = roles.pk_id
        WHERE is_active = 1 ORDER BY username";
        //query result
        return $this->query($strSql);
