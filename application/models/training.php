...
class training extends Base_model {

    // table name
    protected static $table_name = "trainings_record";
    //db fileds
    protected static $db_fields = array('title', 'training_date', 'is_active');
    public $pk_id;
    public $title;
    ...
    public function find_all() {
        if ($_SESSION['role'] == 1) {
            return $this->query("SELECT * FROM " . static::$table_name . " where is_active = 1 order by title");
        } else {
            return $this->query("SELECT * FROM " . static::$table_name . " where is_active =1 order by title");
        }
    }
    ...
    public function trainingdropdown() {

        $sql = "SELECT * FROM `trainings_record` ";
    ...
    public function find_by_personal($id = 0) {
        ...
        trainings_record.pk_id,
        trainings_record.title,
        trainings_record.training_date

        FROM
        personal_trainings
        INNER JOIN trainings_record ON personal_trainings.training_id = trainings_record.pk_id
        WHERE
